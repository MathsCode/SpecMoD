    ...
    def __init__(self, spec_model, ori_model):
        ...
        with torch.no_grad():
            for dataset in datasets:
                file_path = f"/inspire/hdd/project/inference-chip/xujiaming-253308120313/Paper/SpecMoD/train_data/global_router/llama/{dataset}_Llama3.1-8B-Instruct_last_hidden_states_None_None.pt"
                if not os.path.exists(file_path):
                    continue
                last_hidden_states = torch.load(file_path)
                with open(f"/inspire/hdd/project/inference-chip/xujiaming-253308120313/Paper/SpecMoD/train_data/adaptor/0.9/{dataset}_Llama3.1-8B-Instruct_data_None_None.json", "r") as f:
...
if __name__ == "__main__":
    ...
    Ori_model_path = f"/inspire/hdd/global_public/public_models/meta-llama/Llama-3.1-8B-Instruct/"
    Spec_model_path = "/inspire/hdd/project/inference-chip/xujiaming-253308120313/dataset/models/EAGLE3-LLaMA3.1-Instruct-8B"
    ...
    torch.save(router.state_dict(), f"./checkpoint/global_router/Llama3.1_8B_global_router_{args.dim}_Model{args.type}_non_thinking.pt")
