        ...
        if cnt%10==0:
            with open(out_path, "a", encoding="utf-8") as f:
                for idx, _ in enumerate(model.model.layers):
                    print(f'UntilSample{cnt}-layer{idx}: layer_sim_sum is {model.model.IO_sum[idx]}', file=f)
                layer_sorted_indices = [i for i, _ in sorted(enumerate(range(len(model.model.IO_sum))), key=lambda x: model.model.IO_sum[x[0]], reverse=True)]
                print(f'UntilSample{cnt}-LayerSimSort: {layer_sorted_indices}', file=f)
                formatted_list = [num for num in model.model.IO_avg]
                print(f'UntilSample{cnt}-LayerSimAvg: {formatted_list}', file=f)    
                f.write('\n')
        cnt+=1
...
if __name__ == '__main__':
    ...
    datasets = ["trec", "triviaqa", "gov_report"]
...
#how to run: python ob1.py --model llama
#python ob1.py --model vicuna
#python ob1.py --model internlm
