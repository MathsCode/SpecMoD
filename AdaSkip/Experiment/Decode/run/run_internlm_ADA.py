...
def parse_args(args=None):
    ...
    parser.add_argument('--model', type=str, default="internlm-ADA-DECODE")
...
def load_model_and_tokenizer(path, SKIP_SUB_LAYER_NUM, device):
    tokenizer = InternLMTokenizer.from_pretrained(path, trust_remote_code=True)
    config = MyInternLMConfig.from_pretrained(path, SKIP_SUB_LAYER_NUM=SKIP_SUB_LAYER_NUM)
    model = InternLMForCausalLM.from_pretrained(path, config=config, torch_dtype=torch.bfloat16).to(device)
    model = model.eval()
...
if __name__ == '__main__':
    ...
    SKIP_SUB_LAYER_NUM = args.skip_sub_layer_num
    model_name = args.model + "-SKIP-" + str(SKIP_SUB_LAYER_NUM)
    max_length = model2maxlen["internlm"]
    model_path = model2path["internlm"]
    # Decode
    warmup_dataset = "qasper" # "Anyone is OK"
    target_datasets = ["gov_report", "multi_news"]
    ...
    warmup_and_get_pred(warmup_dataset, target_datasets, dataset2prompt, dataset2maxlen, device, model_name, model_path, output_dir, SKIP_SUB_LAYER_NUM)
