        ...
        if cnt%10==0:
            with open(out_path, "a", encoding="utf-8") as f:
                for idx, _ in enumerate(model.model.layers):
                    print(f'UntilSample{cnt}-layer{idx}: attn_sim_sum is {model.model.IO_attn_sum[idx]}', file=f)
                    print(f'UntilSample{cnt}-layer{idx}: ffn_sim_sum is {model.model.IO_mlp_sum[idx]}', file=f)
                attn_sorted_indices = [i for i, _ in sorted(enumerate(range(len(model.model.IO_attn_sum))), key=lambda x: model.model.IO_attn_sum[x[0]], reverse=True)]
                print(f'UntilSample{cnt}-AttnSimSort: {attn_sorted_indices}', file=f)
                formatted_list = [num for num in model.model.IO_attn_avg]
                print(f'UntilSample{cnt}-AttnSimAvg: {formatted_list}', file=f)
                mlp_sorted_indices = [i for i, _ in sorted(enumerate(range(len(model.model.IO_mlp_sum))), key=lambda x: model.model.IO_mlp_sum[x[0]], reverse=True)]
                print(f'UntilSample{cnt}-FFNSimSort: {mlp_sorted_indices}', file=f) 
                formatted_list = [num for num in model.model.IO_mlp_avg]
                print(f'UntilSample{cnt}-FFNSimAvg: {formatted_list}', file=f)  
                f.write('\n')
        cnt+=1
...
#how to run: python ob2.py --model internlm
#python ob2.py --model llama
#python ob2.py --model vicuna